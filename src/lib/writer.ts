...
function toCodeFromEntity(ent: Entity) {
  // TS interface fields
...
function mongooseField(f: Entity["fields"][number]) {
  ...
  switch (f.type) {
    ...
    case "enum":
      if (f.enumValues?.length) return `{ type: String, enum: [${f.enumValues.map(v => `"${v}"`).join(", ")}]${baseReq} }`;
